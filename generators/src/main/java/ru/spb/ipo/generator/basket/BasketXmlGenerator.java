...
public class BasketXmlGenerator extends BaseGenerator {
	...
	public String generateDescription() {
		String res = "";
		if (!isSingle()) {
			res = "»меютс€ две урны A и B , в урне A находитс€ " + toDesc((ComplexElement []) sourceParams.get("basket1")) + " шаров, в урне B - " + toDesc((ComplexElement []) sourceParams.get("basket2")) + ".";  
		} else {
			res = "¬ урне A находитс€ " + toDesc((ComplexElement []) sourceParams.get("basket1")) + " шаров.";
		}
		res += " — какой веро€тностью из урны A можно " + (isCont() ? "последовательно" : "одновременно") + " вытащить" + taskParams.get("inlineDesc") + ".";
		return res;
